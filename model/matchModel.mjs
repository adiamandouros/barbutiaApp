import { FutureMatch, CompletedMatch } from './dbmodel.mjs'
...
export async function addCompletedMatchToDB(match) {
    try {
        ...
        if (!testMatch)
            // throw new Error("There is already a match with the same date!")
            testMatch = await CompletedMatch.create({ teamName: match.teamName, teamLogo: match.teamLogo, league: match.league, date: match.date, place: match.place, homeTeamScore: match.homeTeamScore, awayTeamScore: match.awayTeamScore, isWin: match.isWin, isHome: match.isHome })
        else
